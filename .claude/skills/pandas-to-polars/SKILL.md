...
description: Convert pandas code to Polars for better performance. Use when working with dataframes over 1M rows or when the user asks for a Polars version.
---
...
- For merges, use `join()` with explicit `on=`, `how=` parameters. Note: Polars uses `"left"`, `"inner"`, `"full"` (not `"outer"`).
- Replace `pd.read_csv()` with `pl.read_csv()` or `pl.scan_csv()`. For Stata .dta files, use `pl.from_pandas(pd.read_stata(...))` since Polars has no native .dta reader.
